      ...
      <section style="padding: 10px 14px 14px 14px">
        <div ref="audioHeadRef" class="audio-box__header">
          <div class="audio-name">
            ...
            <span>{{ props.data?.name }}.{{ props.data?.ext }}</span>
          </div>
...
onMounted(() => {
  ...
  window.addEventListener('close-current-audio-modal', handler);

  onBeforeUnmount(() => {
    window.removeEventListener('close-current-audio-modal', handler);
  });
})
