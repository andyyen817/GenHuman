  ...
  <a-modal v-model:visible="visible" :title="title" width="90%" :modal-style="{ maxWidth: '625px' }"
    ...
    <a-form ref="formRef" :model="form" :rules="formRules" auto-label-width>
      ...
      <a-form-item v-if="form.type === 2" label="组件路径" field="component">
        <a-input v-if="isExternalUrl" v-model.trim="form.component" placeholder="请输入组件路径" allow-clear :max-length="50" />
...
import { type MenuItem, getMenuDetail, saveMenuDetail } from '@/apis/permission'
...
const save = async () => {
  try {
    ...
    const res = await saveMenuDetail(form)
    if (res) {
